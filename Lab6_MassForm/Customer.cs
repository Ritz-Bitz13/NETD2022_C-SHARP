...
namespace Lab6_MassForm
{
    ...
    class Customer
    {
        ...
        public static void FileWrite(List<Customer> customers, String filename)
        {
            ...
            sw.Write(FileCreateCSV(customers));
            sw.Close();
        }

        private static String FileCreateCSV(List<Customer> customers)
        {
            String rs = string.Empty;

            foreach(Customer c in customers)
            {
                rs += c.customerTitle.ToString() + ","
                + c.customerFirstName.ToString() + ","
                + c.customerLastName.ToString() + ","
                + c.customerVipStatus.ToString() + "\n";
